...
class SalesOrder(models.Model):
    ...
    is_associated_product = fields.Boolean(string="Associated product",
                                           default=False)

    @api.onchange('is_associated_product', 'partner_id')
    def _onchange_associated_product(self):
        """ If the associated product is true it will add the associated product
        inside the customer form to order line """
