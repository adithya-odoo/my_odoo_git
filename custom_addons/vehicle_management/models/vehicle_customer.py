...
class VehicleCustomer(models.Model):
    ...
    customer_state = fields.Selection(selection=[('non service customer',
    ...
    def _compute_vehicle_history(self):
        """
         To compute the number of vehicle number of a customer to set inside
          the smart button
         """
        self.smart_partner = self.env['vehicle.management'].search_count(
    ...
    def action_archive(self):
        ...
        vehicle_customer = self.env['vehicle.management'].search([('partner_id',
                                                                   '=', self.id)])
        if vehicle_customer:
    ...
    def action_unarchive(self):
        ...
        vehicle_customer = self.env['vehicle.management'].search(
                                                  [('partner_id', '=', self.id),
                                                   ('is_active', '=', False)])
        if vehicle_customer:
    ...
    def get_vehicle_management_form_view(self):
        """
        To get the vehicle service form while clicking the 'create service form'
         button inside partner form
        """
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'vehicle.management',
